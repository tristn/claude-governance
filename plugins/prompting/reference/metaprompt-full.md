# Anthropic Metaprompt — Full Reference

Source: https://github.com/anthropics/claude-cookbooks/blob/main/misc/metaprompt.ipynb
Mirrored at: https://platform.claude.com/cookbook/misc-metaprompt

This is the verbatim metaprompt that powers Anthropic's Console **Prompt Generator**.
It's a long multi-shot prompt with worked examples that teaches Claude how to write
high-quality prompt templates for arbitrary tasks.

The original notebook caveats:
- Designed for single-turn question/response prompts (not multi-turn chat).
- Output is a starting point — not guaranteed optimal. Iterate on it.

The `{{TASK}}` placeholder at the bottom is where the user's task description gets
substituted in. The metaprompt then returns:
- `<Inputs>`: variable names the prompt will use
- `<Instructions Structure>`: the planning/scaffolding step
- `<Instructions>`: the final prompt template (this is the deliverable)
